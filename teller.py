...
class teller(employee):
	...
	def ServeCustomer(self, other):
		'''Teller gives the customer the option to check their balance, make a deposit or withdrawal, transfer funds from an investment account to checking account, or make a payment on their outstanding balance.'''
	...
	def __TransferFromInvestmentAccount(self, other):
		'''Helps the customer transfer funds from an investment account to their checking account, for a %5 fee.'''
	...
	def __PaymentOnOutstandingBalance(self, other):
		'''Helps the customer make a payment on their outstanding balance, if they have one.'''
	...
	def talk(self):
		'''Teller will introduce themselves.'''
